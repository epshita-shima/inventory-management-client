...
import { useCreateUserMutation, useGetSingleUserQuery } from "../../../redux/features/user/userApi";
...
import TreeSingleUserView from "./TreeSingleUserView";
  ...
  console.log(singleUser?.menulist)
    const {
      data: userRoleData,
      ...
      function mergeDropdownPermissions(mainDropdown, permissionsDropdown) {
        ...
        return mainDropdown.map((mainItem) => {
          ...
          const permissionsItem = permissionsDropdown?.find(
            (permItem) => permItem && permItem.trackId === mainItem._id
          );
          console.log(permissionsData)
          if (permissionsItem) {
    ...
    console.log( singleUserData != '',singleUser?.menulist)
    const mergedData = mergePermissions(menuItems,  singleUser?.menulist );
